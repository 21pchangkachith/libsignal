...
impl<T: Parameters> DynParameters for T {
    ...
    // Called by Alice on Bob's PQ public key to generate shared secret
    fn encapsulate(
        &self,
        pub_key: &KeyMaterial<Public>,
        csprng: &mut dyn CryptoRng,
    ...
    // Called by Bob with his PQ private key to recover secret (generated by encapsulate)
    fn decapsulate(
        &self,
        secret_key: &KeyMaterial<Secret>,
        ciphertext: &[u8],
...
// Read first byte of serialized PQ pk which tells what KEM to use
...
// Marker trait for public vs secret keys (abstraction)
pub trait KeyKind {
...
impl Key<Public> {
    ...
    pub fn encapsulate<R: CryptoRng>(
        ...
        // Calls the KEM-specific encapsulation algorithm (Kyber1024, MLKEM1024, etc.) using the public key
        let (ss, ct) = self // ss = shared secret, ct = ciphertext
            .key_type
            .parameters()
...
// Similar to Bob receiving Alice's message in X3DH and computing shared secret
impl Key<Secret> {
...
impl<'a> Ciphertext<'a> {
    ...
    /// Create a binary representation of the key that includes a protocol identifier.
    // Allows for Alice to send ct to Bob (or vice versa?)
